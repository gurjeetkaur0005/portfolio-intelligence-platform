...
def main() -> None:
    ...
    if not portfolio_ids:
        st.info(
            "No stored portfolios are available for rebalancing."
        )
        return
    ...
    transaction_cost_rate = st.number_input(
        ...
        format="%.4f",
        help=(
            "0.002 represents a transaction cost rate "
            "of 0.2%."
        ),
    )
    ...
    st.info(
        ...
        "The frontend does not calculate portfolio trades."
    )
    ...
    if run_button:
        ...
        st.session_state["rebalance_run_id"] = (
            run_id_value
        )
